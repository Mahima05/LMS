...
export default function Ljmap({ containerBackgroundColor = '#fff' }) {
  ...
  //           {journeyStartDate ? `Start: ${journeyStartDate}` : ''}{journeyStartDate && journeyEndDate ? '   |   ' : ''}
  //           {journeyEndDate ? `End: ${journeyEndDate}` : ''}
  //         </Text>
  //       )}
  //     </View>
  ...
  //     {/* 🎓 Milestone Images with Bouncy Animation */}
  //     {milestoneImages.map((milestone, index) => {
  //       const bounceTranslate = bounceAnims[index].interpolate({
  //         inputRange: [0, 1],
  //         outputRange: [0, -15],
  //       });
  ...
  //     {/* ⭐ Start date near Stella */}
  //     {stellaPosition && journeyStartDate && (
  //       <View
  //         style={[
  //           styles.dateBadge,
  //           {
  //             bottom: stellaPosition.y + 10 + ELEMENTS_VERTICAL_OFFSET,
  //             right: stellaPosition.x - 100 - ELEMENTS_HORIZONTAL_OFFSET,
  //           },
  //         ]}
  //       >
  ...
  // return (
  //   <View style={containerStyle}>
  //     {/* ⭐ Journey header OUTSIDE the image container */}
  //     <View style={styles.journeyHeaderOutside}>
  //       {journeyName && (
  //         <Text style={styles.journeyTitle}>
  //           {journeyName}
  //         </Text>
  ...
  //       {(journeyStartDate || journeyEndDate) && (
  //         <Text style={styles.journeyDates}>
  //           {journeyStartDate ? `Start: ${formatDate(journeyStartDate)}` : ''}
  //           {journeyStartDate && journeyEndDate ? '   |   ' : ''}
  //           {journeyEndDate ? `End: ${formatDate(journeyEndDate)}` : ''}
  //         </Text>
  //       )}
  //     </View>
  ...
  return (
    <View style={containerStyle}>
      {/* ⭐ Journey NAME above image - separate */}
      {journeyName && (
        <View style={styles.journeyNameBox}>
          <Text style={styles.journeyNameText}>{journeyName}</Text>
        </View>
      )}

      {/* ⭐ Journey DATES above image - separate */}
      {/* {(journeyStartDate || journeyEndDate || journeyName) && (
        <View style={styles.journeyDatesBox}>
          {journeyName && (
            <Text style={styles.journeyNameText}>{journeyName}</Text>
          )}
          <Text style={styles.journeyDatesText}>
            {journeyStartDate ? `Start: ${formatDate(journeyStartDate)}` : ''}
            {journeyStartDate && journeyEndDate ? '   |   ' : ''}
            {journeyEndDate ? `End: ${formatDate(journeyEndDate)}` : ''}
          </Text>
...
//   containerEmbedded: {
//     width: '100%',
//     height: 650, // ⭐ Increased to accommodate header
//     justifyContent: 'flex-start', // ⭐ Changed
//     alignItems: 'center',
//     padding: 0,
//     position: 'relative',
//   },
//   // ⭐ NEW: Header outside image
//   journeyHeaderOutside: {
//     width: '100%',
//     paddingVertical: 8,
...
const styles = StyleSheet.create({
  ...
  containerEmbedded: {
    width: '100%',
    height: 680, // ⭐ Increased for two separate boxes
    justifyContent: 'flex-start',
    alignItems: 'center',
    padding: 0,
    position: 'relative',
